...
@Component({
  ...
  template: `
    <div class="thread">
    <article class="message" [id]="'msg-' + message.id" [class.deleted]="message.is_deleted" [class.flash]="isFlashed()">
      <div class="avatar">
      ...
      <div class="body">
        ...
        <ng-container *ngIf="!editing(); else editForm">
          ...
          <div
            class="attachments"
            *ngIf="message.attachments?.length && !message.is_deleted"
  ...
  styles: [`
    :host {
      ...
      --surface: #12121e; --raised: #16161f; --line: rgba(255,255,255,0.07);
      --text: #eaeaf2; --mut: #7a7a92; --gold: #d4af37;
    }
    .thread { display: block; }
    .replies {
      margin-left: 21px;
      padding-left: 0.85rem;
      border-left: 2px solid rgba(0,255,163,0.18);
    }
    .replies.capped {
      margin-left: 0;
      padding-left: 0;
    ...
    .message { display: grid; grid-template-columns: 42px 1fr; gap: 0.75rem; padding: 0.45rem 0.6rem; border-radius: 8px; border-left: 2px solid transparent; transition: background 0.12s ease, border-color 0.12s ease; }
    .message:hover { background: var(--surface); border-left-color: rgba(0,255,163,0.35); }
    .message.flash { animation: flashHi 2s ease-out; }
    @keyframes flashHi {
      0%, 25% { background: rgba(255,107,107,0.22); border-left-color: #ff6b6b; }
      100% { background: transparent; border-left-color: transparent; }
    }
    .message.deleted { opacity: 0.5; }
    .avatar { width: 42px; height: 42px; border-radius: 12px; background: var(--raised); display: grid; place-items: center; color: var(--accent); font-family: 'Anton', 'Bebas Neue', sans-serif; box-shadow: inset 0 0 0 1px var(--line); }
    ...
    .reply-icon { color: #00ffa3; opacity: 0.7; }
    ...
    .content { margin: 0.2rem 0 0; color: #cfcfe0; white-space: pre-wrap; word-wrap: break-word; line-height: 1.5; }
    ...
    .thumb { padding: 0; border: none; background: #0d0d17; border-radius: 8px; overflow: hidden; cursor: pointer; aspect-ratio: 1 / 1; }
    .thumb img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.18s ease; }
    /* Single image: container shrinks to the image; no letterbox background. */
    ...
    .reactions { display: flex; gap: 0.3rem; margin-top: 0.4rem; flex-wrap: wrap; }
    .reaction { padding: 0.12rem 0.55rem; background: var(--raised); border: 1px solid var(--line); border-radius: 999px; color: #cfcfe0; cursor: pointer; font-size: 0.82rem; transition: border-color 0.12s ease, box-shadow 0.12s ease; }
    .reaction:hover { border-color: var(--accent); box-shadow: 0 0 10px rgba(0,255,163,0.25); }
    .count { font-size: 0.72rem; font-weight: 700; margin-inline-start: 0.25rem; color: var(--accent); }
    .actions { display: flex; gap: 0.5rem; margin-top: 0.4rem; opacity: 0; transition: opacity 0.15s; align-items: center; }
    ...
    .picker {
      ...
      background: #0d0d17; border: 1px solid var(--line); border-radius: 10px; padding: 0.35rem;
      display: flex; gap: 0.15rem; box-shadow: 0 10px 30px rgba(0,0,0,0.55);
    }
    ...
    .ghost { background: transparent; border: none; color: var(--mut); cursor: pointer; font-size: 0.78rem; font-family: 'JetBrains Mono', ui-monospace, monospace; letter-spacing: 0.02em; }
    .ghost:hover { color: var(--accent); }
    textarea { width: 100%; padding: 0.5rem; background: #07070d; color: var(--text); border: 1px solid var(--line); border-radius: 6px; resize: vertical; }
    .edit-actions { display: flex; gap: 0.5rem; margin-top: 0.25rem; }
    button:not(.ghost):not(.reaction):not(.picker button):not(.thumb) { padding: 0.3rem 0.8rem; background: var(--accent); color: var(--accent-d); border: none; border-radius: 6px; cursor: pointer; font-weight: 700; }
  `],
})
