...
@Component({
  ...
  `,
  styles: [`
    :host { --accent: #7c3aed; --accent-s: #a78bfa; --surface: #161228; --raised: #1c1833; --line: rgba(124,58,237,0.18); --text: #eaeaf2; --mut: #7a7a92; }
    ...
    .poll-flag { font-size: 0.62rem; text-transform: uppercase; letter-spacing: 0.08em; color: #fff; background: var(--accent); padding: 0.05rem 0.4rem; border-radius: 999px; font-family: 'JetBrains Mono', ui-monospace, monospace; }
    ...
    .opt:hover:not(:disabled) { border-color: rgba(124,58,237,0.55); background: rgba(124,58,237,0.08); }
    .opt:disabled { cursor: default; }
    .opt.voted { border-color: var(--accent); }
    .bar { position: absolute; left: 0; top: 0; bottom: 0; background: rgba(124,58,237,0.14); transition: width 0.3s ease; z-index: 0; }
    .opt.voted .bar { background: rgba(124,58,237,0.28); }
    .marker {
    ...
    .tick { color: #fff; font-size: 0.7rem; font-weight: 900; line-height: 1; }
    .opt-label { position: relative; z-index: 1; flex: 1; }
    .opt-pct { position: relative; z-index: 1; font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 0.8rem; color: var(--accent-s); font-weight: 700; }
